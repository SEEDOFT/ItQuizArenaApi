...
class NetworkingQuestionSeeder extends Seeder
{
    public function run(): void
    {
        $jsonPath = storage_path('app/private/data/networking.json');

        if (! file_exists($jsonPath)) {
            $this->command->error("Networking JSON file not found at: $jsonPath");
        ...
        if (! $data || ! isset($data['sections'])) {
            $this->command->error('Invalid networking JSON structure');
        ...
        $this->command->info("Seeded {$count} networking questions from JSON.");
    }
}
